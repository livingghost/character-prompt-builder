...
def validate_layout(request:dict,layout:dict,media:list[dict],expected_target:dict) -> None:
    ...
    for slot,value in (('model',expected_target['model_identifier']),('operation',expected_target['operation'])):
        if layout[slot] is not None and get(request,layout[slot])!=value:raise ValueError('the built request changes its declared target')
    ...
    protected=[x for x in (layout['model'],layout['operation'],layout['output_count']) if x is not None]+destinations+[x['field'] for x in layout['content']]
