...
It is an Agent Skill: instructions the agent follows, plus local Python tools the agent runs. The author and the agent decide what an image is for and judge what comes back. The tools search the bundled production knowledge, check inputs, keep files and hashes, rebuild story-time state from recorded events, and track what was approved. Authoring and the offline examples below run without an API key.
...
```text
brief, existing design or approved story-time state
  -> fixed anchors and open visual decisions
  -> direction cards while meaningful decisions stay open
  -> one selected direction
  -> full reading of the production records it uses
  -> applicable state and scene Persona material, when continuity matters
  -> prompt and scoped negative instructions
  -> verified generation package with ordered references
  -> approved execution and every returned candidate
  -> inspection, local repair, selection, and scoped write-back
```

**Settle the intent first.** The author supplies or decides the purpose, the fixed conditions and the acceptable freedom. The agent turns them into a complete rendition: what the viewer should recognize, what carries the composition, how the subject is built, how the finish serves the purpose. When a sparse brief leaves several directions open, the agent compares complete alternatives instead of piling incompatible details into one prompt. A prompt-only request can stop here.

**Read what makes the direction possible.** Catalog search finds production records; the agent then reads the selected records in full, including their constraints and the conditions they apply under. For a recurring character, the Persona, the design, the authorial intent and the story-time state say what stays fixed and what may vary.

**Turn the direction into exact inputs.** The agent decides how each requirement appears in text and references. The scripts check the declared model and service parameters, keep the attachment order and permitted influence of every reference, and bind the package to its actual files. Sending a request is a separate step that needs a prepared production run and an explicit grant.

**Inspect and continue deliberately.** Every returned candidate is kept with the input that produced it. The review compares the actual image with the stated intent, separates what was observed from why it might have happened, and targets a repair at the part that needs changing. Selecting an image for delivery, using it as a reference and adopting a permanent character fact are three separate decisions. A new session resumes from the saved project and its records.
...
Once the core check passes, initialize the content-pack state and ask the catalog for directions around a real brief. These commands read local resources only and generate nothing.
...
`recommend` returns direction cards. One card from that brief, trimmed:

```text
title:      Warm everyday character portrait
preserves:  lighting: overcast light
adds:       quiet interior context, soft focal light, approachable character acting,
            close conversational proximity, soft overcast ambience
adjustable: occupation, casual clothing, room type, gaze, time of day
preset_ids: style-family-polished-soft-cel-portrait, low-angle-apron-work-portrait,
            camera-close-conversational-proximity, lighting-overcast-ambient
open axes:  identity, role, wardrobe, pose, performance, environment, mood
```
...
Everything up to a production run: deciding what the image is for, developing the world and its subjects, reading production knowledge, and preparing reusable character and scene material.

### Creative development and authorial intent

Creator intent is its own record, kept apart from world facts and from a subject's psychology. The [design form](templates/narrative/design/design-template.md) holds the portrayal aims, the recognition targets, the permitted variation and the deliberate departures, each with its scope. The [persona form](templates/narrative/personas/persona-template.md) binds a character's identity profile and its conditional voice and body rules to those aims. Constancy can be the aim, and so can ambiguity. [Authorial Intent](references/runtime/authorial-intent.md) defines the records; [Authoring commands](references/narrative-authoring.md) create the full forms and audit the links between them.
...
3. Choose one coherent rendition and return the prompt with its negative instructions and the choices still open.

For a recurring subject, supply the current Persona, design and state, and say which kind of request it is: a local variation, a redesign, a new story-time condition or an exploratory candidate. A new camera angle leaves anatomy alone, and a new outfit leaves stable identity alone.
...
The order matters: author the structured sheet data, prepare the layout, obtain any generated panel images, then record acceptance after review. Until acceptance, a panel image is a candidate. The sheet is authoring data as well as a renderable layout; generated panels can be cropped and composed into place. Accepting an image for one panel, selecting a production candidate and adopting a permanent identity detail are three decisions with three scopes. A reference board exposes the aspects the requested production needs, sized to the subject rather than to a fixed number of views. [Character Sheet Discipline](references/runtime/character-sheet-discipline.md) gives the panel, mask and board workflow.

### Prepare reusable scene Persona material

Even an uneventful exchange can depend on definitions spread across the whole Persona. During scene preparation the agent reads the complete applicable originals, selects the exact passages the scene needs, and records their conditions, dependencies and application. The result is a document that later writing, repair and resumption reuse within its reviewed scope, instead of guessing the needed sections again.

Try the file workflow in an empty sibling directory. The example's few lines of subject definition demonstrate the data flow, not a real Persona:
...
Preparation verifies the files and carries the text into the consumer input, the exact input handed to the generator. Author-only information stays out of the image prompt. Changing any complete source, quoted or not, invalidates reuse, and a new participant, topic or portrayal aim can call for fresh preparation even when no file changed. When a gap appears, go back to the originals. [Scene Persona Material](references/runtime/scene-persona.md) covers replacement and accepted external snapshots.

### Resolve story-time state and ground it in the world

The state resolver rebuilds story-time state from declared events and processes, keeping stable identity, temporary state, the visible projection and the observed output apart. [World Realization](references/runtime/world-realization.md) connects pinned source evidence, event resolution, authorial intent and recipient-specific views, and can inspect, build, verify and report source-change impact without a model or a cast. [State-aware authoring](references/runtime/state-aware-series.md) covers the path; the [unpeopled worked example](examples/world-realization/README.md) is runnable.
...
Turning an approved direction into an exact, approved request: the work ledger, the dispatcher, Studio review, local editing and reference transport.

### Prepare and record a production run

Any task that produces a saved artifact runs through the work ledger and the production workflow. The task record names its purpose, source decisions, intended expression, output kind, review criteria and the scope of the authority delegated for it. Preparation freezes those inputs into a run; handoff, external execution, review, selection and adoption follow as distinct operations, each leaving a record.

```text
brief and complete applicable sources
  -> authored direction and scene material
  -> prepared run and exact consumer input
  -> approved handoff and, when needed, approved submission
  -> captured candidates and located observations
  -> reviewed selection, optional canonical adoption, completion
```
...
The example's approvals are labeled synthetic fixtures. Real work uses your own grants. [Production Execution](references/runtime/production-execution.md) gives the task, authorization and review commands.

### Send an approved image generation

A live generation needs a configured service, a model record that describes that service's fields, credentials outside the project files, a prepared production run and a package bound to that run. The bundled transport is Runware; another service needs its own transport or an explicitly recorded external handoff. A dry run shows the exact request:

```sh
python scripts/dispatch.py generation-package.json --studio PROJECT --character SUBJECT_ID --slot SLOT_ID --production-root PROJECT --production-run RUN_ID
```

Approve the submission intent the dry run prints, with the real output count and cost bound. The live send then carries the receipt, the recorded approval:
...
An upscale is its own prepared and approved task binding the source image, the upscaler, the factor and the settings, with its own receipt:

<!-- readme-send: upscale -->
```sh
python scripts/dispatch.py --upscale --model UPSCALER_ID --source SOURCE_IMAGE --scale 2 --studio PROJECT --character SUBJECT_ID --slot SLOT_ID --production-root PROJECT --production-run RUN_ID --production-authorization RECEIPT_SHA --send
```
<!-- end-readme-send -->
...
A production review records what was seen, where, how it was interpreted and what remains uncertain. A failed hard criterion blocks selection, and a changed candidate or task gets its own review. The HTML review export puts the recorded inputs, the consumer input, all candidates, the observations and the selection reasons in one inspectable page. [Evidence Review](references/runtime/evidence-review.md) explains the export and the optional study built on it.

### Investigate repeated failures

A repair names the observed problem, the proposed change and the requirements that stay fixed. When failures repeat, [Repair Analysis](references/runtime/repair-analysis.md) groups them across runs by the criterion's actual definition and writes a readable report: the source run's freshness, its review receipt, the cited observations with their locations, and the reviewer's reason. A cause hypothesis carries its evidence, alternatives, protected requirements, scope and acceptance checks, and stays a proposal until a review confirms it.
...
Use explicit `--state-file`, `--cache-dir` and `--managed-root` locations to isolate a project or a test, and enable an additional pack by its discovered UUID. A missing model or vocabulary record usually means the pack is disabled or its provider unresolved. [Content Packs](PACKS.md) explains activation and resource providers; the core ships the minimal [commons pack](packs/commons/).
...
Keep projects and generated outputs outside the installed skill folder. Content-pack activation persists under the user's home directory and survives skill updates. A separately supplied user library is additional content.
...
**Sending is refused.** Check the prepared run, the exact approval, the service configuration and the credentials. The operation, input, output count and cost must fit the grant; `--send` alone grants nothing. An unknown remote result needs investigation before a new submission.

**Catalog resources are missing.** Check discovered packs, enabled UUIDs and the selected resource provider. A folder on disk is active only once its pack is enabled.
